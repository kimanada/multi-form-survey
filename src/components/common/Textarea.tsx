...
const Textarea = forwardRef<HTMLTextAreaElement, TextareaHTMLAttributes<HTMLTextAreaElement>>(function TextArea({ className, ...props }, ref) {
  ...
  <textarea 
    ...
    className={cn(
      ...
      "focus:border-b-gray600 focus:bg-bg2 focus:rounded-t-6", className)} {...props} />
  );
});

export default Textarea;

export function AutoGrow({ value, className, forTextarea = "", ...props }: HTMLAttributes<HTMLDivElement> & { value?: string, forTextarea?: string }) {
